...
static double gXErr = 0, gYErr = 0;

void pe_inertial_navigation(PostureStruct *ps)
{
    double err = 0.0001;
    double speX, speY, gX, gY, aX, aY;
    ...
    // adjust
    if (gX > err) gXErr -= err;
    else if (gX < -err) gXErr += err;
    if (gY > err) gYErr -= err;
    else if(gY < -err) gYErr += err;
#endif
    aX = gX * PE_GRAVITY / PE_MASS;
    aY = gY * PE_GRAVITY / PE_MASS;
    //g值积分得到速度
    speX = ps->speX;
    speY = ps->speY;
    speX += SPE_SUN_FUN(aX, ps->aX);
    speY += SPE_SUN_FUN(aY, ps->aY);
    //速度积分得到移动距离
    ...
    ps->speY = speY;
    //
    dot_set(gX, 0, 0xFF0000);
    dot_set(0, gY, 0x0000FF);
    dot_set(gX, gY, 0xFFFF00);
}
